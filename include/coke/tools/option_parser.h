...
public:
    ...
    bool has_long_name() const noexcept {
        return long_name != NULL_LONG_NAME;
    }
...
public:
    NoSuchOptionError(char short_name)
        : OptionError(short_name, "")
    {
        error.assign("No short option named -").push_back(short_name);
    }

    NoSuchOptionError(const std::string &long_name)
        : OptionError(long_name, "")
    {
...
public:
    OptionNotSetError(char short_name)
        : OptionError(short_name, "")
    {
    ...
    OptionNotSetError(const std::string &long_name)
        : OptionError(long_name, "")
    {
...
protected:
    OptionBase(char short_name, const std::string &long_name,
...
template<typename T>
    requires std::integral<T>
using MIntegerOption = BasicOption<std::vector<T>,
                                   MultiOptionTrait<IntegerOptionTrait<T>>>;
...
template<typename T>
    requires std::floating_point<T>
using MFloatingOption = BasicOption<std::vector<T>,
                                    MultiOptionTrait<FloatingOptionTrait<T>>>;
...
using MStringOption = BasicOption<std::vector<std::string>,
                                  MultiOptionTrait<StringOptionTrait>>;
...
public:
    ...
    */
    ...
    add_integer(T &val, char short_name, const std::string &long_name,
        ...
        return add<IntegerOption<T>>(&val, short_name, long_name,
                                     required, desc);
    }
    ...
    */
    ...
    add_multi_integer(std::vector<T> &val,
                      char short_name, const std::string &long_name,
                      bool required = false, const std::string &desc = "") {
        return add<MIntegerOption<T>>(&val, short_name, long_name,
                                      required, desc);
    }
    ...
    */
    ...
    add_floating(T &val, char short_name, const std::string &long_name,
                 bool required = false, const std::string &desc = "") {
        return add<FloatingOption<T>>(&val, short_name, long_name,
                                      required, desc);
    }
    ...
    */
    ...
    add_multi_floating(std::vector<T> &val,
                       char short_name, const std::string &long_name,
                       bool required = false, const std::string &desc = "") {
        return add<MFloatingOption<T>>(&val, short_name, long_name,
                                       required, desc);
    }
    ...
    */
    ...
    add_countable_flag(int &val, char short_name, const std::string &long_name,
                       const std::string &desc = "") {
        auto &opt = add<CountableOption>(&val, short_name, long_name,
                                         false, desc);
        opt.set_has_value(false);
        val = 0;
        return opt;
    ...
    *        is 1024, which means 1KB = 1024B. Multi unit can join togegher, for
    *        example, 1M2KB3B.
    * Both uppercase and lowercase are acceptable. If the unit is not specified,
    * the default is B. Negative numbers are not allowed.
    */
    ...
    add_data_unit(T &val, char short_name, const std::string &long_name,
                  bool required = false, const std::string &desc = "") {
        return add<DataUnitOption<T>>(&val, short_name, long_name,
                                      required, desc);
    }
