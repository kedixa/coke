...
public:
    ...
    {
        return check_type(MYSQL_TYPE_TINY, MYSQL_TYPE_SHORT, MYSQL_TYPE_INT24,
    ...
    {
        return check_type(MYSQL_TYPE_DECIMAL, MYSQL_TYPE_NEWDECIMAL);
    }
    bool is_string() const
    {
        return check_type(MYSQL_TYPE_VARCHAR, MYSQL_TYPE_VAR_STRING,
                          MYSQL_TYPE_STRING);
    }
    bool is_blob() const
    {
        return check_type(MYSQL_TYPE_TINY_BLOB, MYSQL_TYPE_MEDIUM_BLOB,
    ...
    {
        return as_numeric<unsigned long long>(0);
    }
...
public:
    MySQLFieldView()
        : length(0), flags(0), decimals(0), charsetnr(0),
...
public:
    ...
    bool is_ok() const
    {
        return result_set && result_set->type == MYSQL_PACKET_OK;
    }

    bool is_result_set() const
    {
        return result_set && result_set->type == MYSQL_PACKET_GET_RESULT;
    }

    bool is_error() const
    {
        return result_set && result_set->type == MYSQL_PACKET_ERROR;
    }
    ...
    {
        return result_set->affected_rows;
    }
...
class MySQLResultSetCursor {
    struct iterator {
        ...
        ~iterator() { mysql_result_set_cursor_deinit(&cursor); }
