...
SleepBase::SleepBase(SleepBase &&that) noexcept
    : AwaiterBase(std::move(that)), timer(std::exchange(that.timer, nullptr)),
...
SleepAwaiter::SleepAwaiter(double sec) : SleepAwaiter(to_nsec(sec)) {}
...
SleepAwaiter::SleepAwaiter(const void *addr, InfiniteDuration, bool insert_head)
{
